...
import { ClipboardCheck, Loader2, Mail, Eye, EyeOff } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Keep loading state - redirect will happen via useEffect
    } catch (err: unknown) {
        ...
        <div className="w-full rounded-3xl border border-zinc-200 bg-white/80 backdrop-blur-sm p-8 shadow-xl">
          ...
          <div className="mt-8">
            <Button
              type="button"
              variant="outline"
              className="w-full h-12 text-base font-semibold text-zinc-800 border-zinc-300 hover:bg-zinc-50 hover:border-zinc-400"
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            <Button
              type="submit"
              className="w-full h-12 text-base font-medium bg-gradient-to-r from-emerald-500 to-cyan-500 hover:from-emerald-600 hover:to-cyan-600"
              disabled={isLoading || isGoogleLoading}
            >
              {isLoading ? (
                <Loader2 className="mr-2 h-5 w-5 animate-spin" />
